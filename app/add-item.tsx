...
function classifyImage(category: ItemCategory): { subType: string; colorFamily: string } {
  ...
  const pickImage = async (useCamera: boolean) => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        const classification = classifyImage(category);
        setSubType(classification.subType);
        setColorFamily(classification.colorFamily);
        setStep(1);
      }
    } catch (e) {
    ...
    <View style={[styles.container, { paddingTop: insets.top + webTopInset }]}>
      ...
      {step === 1 && (
        <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 20) + (Platform.OS === 'web' ? 34 : 0) }]}>
          <Pressable
            style={[styles.saveButton, !photoUri && { opacity: 0.4 }]}
            onPress={handleSave}
            disabled={!photoUri}
          >
...
const styles = StyleSheet.create({
  ...
  photoPreview: { width: '100%', aspectRatio: 0.75, borderRadius: 16, overflow: 'hidden', marginBottom: 20, position: 'relative' },
